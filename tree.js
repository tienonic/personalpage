(function(){
  ...
  function drawTree(growth){
    ...
    for(var di=0;di<depths.length;di++){
      ...
      var dr=depth/(maxD||1);
      var thick=Math.max(0.15,2.5*Math.pow(thickDecay,depth));
      ctx.strokeStyle="rgba(25,22,18,"+(0.92-dr*0.25)*g+")";
      ctx.lineWidth=thick;
      var chs=chains[depth];
      for(var ci=0;ci<chs.length;ci++){
        var chain=chs[ci];
        var pts=[];
        pts.push({x:chain[0].x1*sc+ox,y:chain[0].y1*sc+oy});
        for(var si=0;si<chain.length;si++){
          var s=chain[si];
          var ex=(s.x1+(s.x2-s.x1)*g)*sc+ox;
          var ey=(s.y1+(s.y2-s.y1)*g)*sc+oy;
          pts.push({x:ex,y:ey});
        }
        ctx.beginPath();
